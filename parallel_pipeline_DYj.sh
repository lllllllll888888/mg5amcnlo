#!/usr/bin/env bash
# =============================================================================
# Parallel LHE Pipeline for Z+jets EW Sudakov Validation (v2)
# =============================================================================
# Runs TWO parallel pipelines from the same input LHE:
#   Path A: EW Sudakov reweight -> shower (QCD+QED, qed_shower=T)
#           Produces NLO_QCD+PS+EWS HepMC with 2xxx Sudakov weight columns.
#   Path B: shower only (QCD-only, qed_shower=F), NO reweight
#           Produces NLO_QCD+PS baseline for ratio comparisons against A.
#
# Both paths consume the same chunks (per-event index alignment preserved),
# so an event-by-event ratio between A and B is well-defined.
#
...
#   DEBUG_PATH_A_REWEIGHT=false  # If true, run a single input LHE through reweight only (no shower)
#   DEBUG_MAX_EVENTS=1000     # In debug reweight mode, build/use a reduced LHE with first N events (0 = all)
...
#   QCUT=20 ./parallel_pipeline_DYj.sh events.lhe.gz
...
# Default: "1 1.5 2" → 3 ξ × 5 NLL variants = 15 Sudakov weight columns per FxFx scale.
...
# Shower card generator. Pass "T" for path A (QCD+QED) or "F" for path B (QCD-only).
...
#  Generated by parallel_pipeline_DYj.sh (qed_shower=${qed_flag})      *
#***********************************************************************
nevents      = -1
nsplit_jobs  = 1
combine_td   = T
...
# Write cards into a directory for the given path:
#   path "a": reweight_card.dat + shower_card.dat (qed=T)
#   path "b": shower_card.dat (qed=F) only
write_cards_to_dir() {
    local cards_dir="$1" path="$2"

    case "$path" in
        a)
            echo "$REWEIGHT_CARD_TEMPLATE" > "$cards_dir/reweight_card.dat"
            generate_shower_card T > "$cards_dir/shower_card.dat"
            log_debug "write_cards: path_a (reweight + shower qed=T) -> $cards_dir"
            ;;
        b)
            rm -f "$cards_dir/reweight_card.dat"
            generate_shower_card F > "$cards_dir/shower_card.dat"
            log_debug "write_cards: path_b (shower qed=F, no reweight) -> $cards_dir"
            ;;
        *)
            log_error "write_cards: unknown path '$path' (expected a or b)"
            return 1
            ;;
    esac
}
...
create_shadow_procnlo() {
    local shadow="$1" path="$2"
    local source="$PROC_DIR"

    log "Creating shadow PROCNLO (path_${path}): $shadow ..."
    ...
    write_cards_to_dir "$shadow/Cards" "$path"
    case "$path" in
        a) log "  Wrote cards: shower (Qcut=$QCUT, njmax=$NJMAX, qed=T) + reweight (include_sudakov=True)" ;;
        b) log "  Wrote cards: shower (Qcut=$QCUT, njmax=$NJMAX, qed=F), no reweight" ;;
    esac
...
EOF
    ...
    # MG5 can exit 0 even on failure — check log for known silent-failure
    # signatures. Each pattern below was observed in failed-worker logs
    # during the 2026-05-19 emission-pathways audit:
    #   - "interrupted with error"               outer aMCatNLO dispatcher
    #   - "Reweighting failed"                   common_run_interface.py:2194
    #   - "Some of the run failed"               common_run_interface.py:2284
    #   - "[FATAL] _prepare_fxfx_sudakov_inputs" fxfx_ewsudakov.py:3668 (precedes sys.exit(3))
    #   - "Segmentation fault" / "f2py crash"    MadLoop native crashes
    local reweight_fail_re='interrupted with error|Reweighting failed|Some of the run failed|\[FATAL\] _prepare_fxfx_sudakov_inputs|Segmentation fault|f2py crash'
    local matched_signature
    if matched_signature=$(grep -m1 -E "$reweight_fail_re" "$logdir/reweight.log"); then
        log_error "reweight: log matched silent-failure signature: ${matched_signature}"
        log_error "  See $logdir/reweight.log"
        return 1
    fi
    ...
    # Content sanity check: every event in a successful EW-Sudakov reweight
    # carries a <wgt id='2001'> entry (NLL central, ξ=1.0). If an inner-MG5
    # multi-core sub-worker silently failed, its slice was never reweighted
    # and the un-reweighted LHE was left on disk by reweight_interface.py
    # (the files.mv commit at line 813 is never reached). Downstream this
    # surfaces as ~30% of HepMC events lacking EWSL columns, faking a flat
    # 28% inclusive σ suppression in candy plots. Cf. audit R1 in
    # docs/notes/2026-05-19-audit-mg5-reweight-emission-pathways.md.
    local n_events n_ewsl
    n_events=$(grep -c '^<event' "$procnlo/Events/run_01/events.lhe")
    n_ewsl=$(grep -cE "<wgt id=['\"]2001['\"]" "$procnlo/Events/run_01/events.lhe")
    if (( n_events == 0 )); then
        log_error "reweight: output LHE has zero <event> blocks"
        log_error "  See $logdir/reweight.log"
        return 1
    fi
    if (( n_ewsl != n_events )); then
        log_error "reweight: only ${n_ewsl}/${n_events} events carry <wgt id='2001'> — silent reweight failure (audit R1: inner-MG5 sub-worker emitted un-reweighted chunk)"
        log_error "  See $logdir/reweight.log"
        return 1
    fi
...
# Path B: shower only (QCD shower, qed_shower=F) — no reweight, baseline NLO QCD
run_path_b_worker() {
    local id="$1" chunk="$2" worker_dir="$3"
    local procnlo="$worker_dir/procnlo"
    local logfile="$worker_dir/worker.log"
    ...
    {
        log_step "B-$id" "START" "shower(QCD-only, qed=F)"
        log_step "B-$id" "INPUT" "$(basename "$chunk") ($(count_events "$chunk") events)"

        local step_start; step_start=$(date +%s)
        if ! run_step "shower" "$worker_dir" \
            process_shower "$chunk" "$worker_dir/output/qcdonly.hepmc.gz" "$procnlo" "$worker_dir"; then
        ...
        local total_elapsed; total_elapsed=$(get_elapsed "$worker_start")
        echo "$worker_dir/output/qcdonly.hepmc.gz" > "$worker_dir/final_output.txt"
        echo "SUCCESS:$(format_duration "$total_elapsed")" > "$worker_dir/status"
...
export -f run_path_a_worker run_path_a_reweight_debug_worker run_path_b_worker
...
generate_summary() {
...

    {
        ...
        if [[ -d "$work_dir/path_a/workers" ]]; then
            echo "--- Path A: EW Sudakov reweight → shower (QCD+QED) ---"
            local sc=0 fc=0
            for sf in "$work_dir/path_a/workers/"*/status; do
                [[ -f "$sf" ]] || continue
                local s; s=$(cat "$sf")
        ...
        if [[ -d "$work_dir/path_b/workers" ]]; then
            echo "--- Path B: shower only (QCD-only, qed_shower=F) ---"
            local sc=0 fc=0
            for sf in "$work_dir/path_b/workers/"*/status; do
                [[ -f "$sf" ]] || continue
                local s; s=$(cat "$sf")
                if [[ "$s" == SUCCESS* ]]; then ((sc++)); echo "  $(basename "$(dirname "$sf")"): $s"
                elif [[ "$s" == FAILED* ]]; then ((fc++)); echo "  $(basename "$(dirname "$sf")"): $s"
...
main() {
    ...
    mkdir -p "$work_dir"/{master,chunks,path_a,path_b}
    ...
    echo ""
    echo "==========================================================="
    echo " EW Sudakov Z+jets Pipeline v2 (paths A + B)"
    echo "==========================================================="
    echo " Input:            $INPUT_LHE"
    echo " Total events:     $nevents"
    echo " Events per chunk: $EVENTS_PER_CHUNK"
    echo " Number of chunks: $nchunks  (× 2 paths = $((2 * nchunks)) workers)"
    ...
    echo "==========================================================="
    echo ""
    echo " Path A: reweight(EW Sudakov) → shower(QCD+QED, qed=T)"
    echo " Path B: shower only          (QCD-only,    qed=F)"
    echo ""

    if [[ "$DRY_RUN" == "true" ]]; then
        echo "[DRY RUN] Would process $nchunks chunks across $((2 * nchunks)) workers (A + B)"
        return 0
    fi

    # Split master LHE once; both paths read the same chunks → per-event alignment
    log "Splitting into $nchunks chunks (shared by A and B)..."
    split_lhe "$work_dir/master/events.lhe" "$nchunks" "$work_dir/chunks"

    # One shadow PROCNLO per path (different cards)
    log "Creating shadow PROCNLO directories..."
    create_shadow_procnlo "$work_dir/path_a/shadow_procnlo" "a"
    create_shadow_procnlo "$work_dir/path_b/shadow_procnlo" "b"

    # Worker directories for both paths
    log "Setting up worker directories..."
    for i in $(seq 0 $((nchunks-1))); do
        local id; id=$(printf %03d "$i")
        create_worker_procnlo "$work_dir/path_a/workers/worker_$id" "$work_dir/path_a/shadow_procnlo" "$i"
        create_worker_procnlo "$work_dir/path_b/workers/worker_$id" "$work_dir/path_b/shadow_procnlo" "$i"
    done

    local total_workers=$((2 * nchunks))
    ...
    for i in $(seq 0 $((nchunks-1))); do
        local id; id=$(printf %03d "$i")
        local chunk="$work_dir/chunks/chunk_$id.lhe"

        # Concurrency gate before path A launch
        ...
        # Concurrency gate before path B launch
        while [[ ${#active_pids[@]} -ge $MAX_CONCURRENT ]]; do
            local new_pids=()
            for pid in "${active_pids[@]}"; do
                if kill -0 "$pid" 2>/dev/null; then
    ...
    # Count results
    local a_success a_failed b_success b_failed
    ...
    echo ""
    log "Merging path A outputs..."
    mkdir -p "$work_dir/path_a/output"
    merge_hepmc "$work_dir/path_a" "$work_dir/path_a/output/Zj_FxFx_EWS_qcut${QCUT}.hepmc.gz"

    log "Merging path B outputs..."
    mkdir -p "$work_dir/path_b/output"
    merge_hepmc "$work_dir/path_b" "$work_dir/path_b/output/Zj_FxFx_QCDshower_qcut${QCUT}.hepmc.gz"
    ...
    echo "==========================================================="
    echo " Path A (EWS + QCD+QED shower): $a_success/$nchunks ok, $a_failed failed"
    echo " Path B (NLO QCD + QCD shower): $b_success/$nchunks ok, $b_failed failed"
    echo ""
    echo " Output A: $work_dir/path_a/output/Zj_FxFx_EWS_qcut${QCUT}.hepmc.gz"
    echo " Output B: $work_dir/path_b/output/Zj_FxFx_QCDshower_qcut${QCUT}.hepmc.gz"
