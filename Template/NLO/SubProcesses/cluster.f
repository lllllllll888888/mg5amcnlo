...
C given by the to_mconfigs common block.
     ...
     $     ,cluster_conf,iconf,cluster_ij(max_branch)
     $     ,cluster_par_i(max_branch),cluster_par_j(max_branch)
     $     ,iord(0:max_branch),nqcdrenscale,iconfig,need_matching(nexternal)
     ...
     $     ,nexternal)
      logical firsttime(0:fks_configs),skip_first,skip_first_write
       CHARACTER*(2000) clusterstring
       COMMON /c_clusterstring/ clusterstring
c New array to store clustering strings for all iproc within one event
       INTEGER MAXFKSLOC
       PARAMETER (MAXFKSLOC=512)
       CHARACTER*(2000) clusterstrings(0:MAXFKSLOC)
       COMMON /c_clusterstrings/ clusterstrings
      INTEGER iclus,isave
      INTEGER mother_pdg
      CHARACTER*32 float,stepid,integf
      CHARACTER*32 momstr
      CHARACTER*32 cidstr
      CHARACTER*8 role
      CHARACTER*200 temp
      CHARACTER*64 temp0
      CHARACTER*50 pairstr
      CHARACTER*6  tmpidx
      INTEGER cid, cidlocal, bitpos
      integer get_step_mother_pdg
       data (firsttime(i),i=0,fks_configs) /nfks1*.true./
      save ipdg,cluster_list,cluster_pdg,cluster_type,firsttime
      data temp0 /'<clustering'/
...
c cluster_ij) with scales (in cluster_scales)
     ...
     $     ,cluster_conf,cluster_scales,cluster_ij,cluster_par_i
     $     ,cluster_par_j,iord,cluster_type(1,iproc))
...
c form factor and renormalisation and factorisation scales.
      ...
      ! For writeout, do not skip the first clustering so that the number of
      ! printed steps reflects the full clustering history
     ...
     $     ,need_matching(1))
      !--------------------------------------------------------------
      ! Build clustering-history string compliant with LHE v3.0:
      !  <clustering step_1="i,j,scale" … final_scale="scale"/>
      !  where i and j are external‐particle indices merged at that step.
      !--------------------------------------------------------------
      clusterstring=''
      temp=''
      if (iproc.eq.0) then
      ...
      write(cidstr,'(I0)') iproc
      temp0='<clustering type="'//trim(adjustl(role))//'" iproc="'//
     $       trim(adjustl(cidstr))//'"'

      isave = 0   ! counter for steps that are actually written (final-state only)
      do iclus = 1, nbr
         if (skip_first_write .and. iclus.eq.1) cycle

         ! Increment visible step counter and write it
         isave = isave + 1
         write(integf,'(I0)') isave

         ! Format the scale
      write(float,'(ES20.10)') cluster_scales(iclus)
         float = trim(adjustl(float))

         ! Decode the bitmask in cluster_ij to external indices
         pairstr=''
         cid=cluster_ij(iclus)
         bitpos=1
         cidlocal=cid
         do while (cidlocal.gt.0)
            if (btest(cidlocal,0) .and. bitpos.le.next) then
               write(tmpidx,'(I0)') bitpos
               pairstr=trim(pairstr)//','//trim(adjustl(tmpidx))
            endif
            cidlocal = ishft(cidlocal,-1)
            bitpos = bitpos + 1
         enddo
         if (len_trim(pairstr).ge.1) pairstr=pairstr(2:)  ! drop first comma

         ! Determine mother PDG for this clustering step using mapping iord
         mother_pdg = get_step_mother_pdg(iclus, nbr, iord, cluster_pdg(il_pdg))
         write(momstr,'(I0)') mother_pdg
         temp = trim(temp)//' step_'//trim(adjustl(integf))//'="'//trim(pairstr)//';'//
     $         trim(float)//';mom='//trim(adjustl(momstr))//'"'
      enddo

      write(float,'(ES20.10)') cluster_scales(0)
      float = trim(adjustl(float))
      temp = trim(temp)//' final_scale="'//trim(float)//'"'

      clusterstring = trim(adjustl(trim(temp0)//' '//trim(temp)//'></clustering>'))
      clusterstrings(iproc) = clusterstring
      return
      end
...
c respectively)
     ...
     $     ,mapconfig
      ...
      integer function get_step_mother_pdg(iclus, nbr, iord, cluster_pdg)
c Returns the PDG code of the mother for clustering step 'iclus'
      implicit none
      integer iclus, nbr
      integer iord(0:nbr)
      integer cluster_pdg(0:2,0:2*nbr)
      get_step_mother_pdg = cluster_pdg(0,iord(iclus))
      return
      end
...
CCCCCCCCCCCCCCC-- MAIN CLUSTER ROUTINE -- CCCCCCCCCCCCCCCC
     ...
     $     ,cluster_scales,cluster_ij,cluster_par_i,cluster_par_j,iord
     $     ,cluster_type)
...
c clustering scales (cluster_scales).
     ...
     $     ,cluster_ij(nbr),cluster_par_i(nbr),cluster_par_j(nbr)
     $     ,cluster_pdg(0:2,0:2*nbr,nconf),iord(0:nbr)
...
c Combine the momenta (and update particle types)
     ...
     $        ,iclus),is_bw,particle_type,cluster_type(win_id))
         ! Save the two parent masks before updating imap
         cluster_par_i(iclus)=imap(iwin)
         cluster_par_j(iclus)=imap(jwin)
