...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      integer i,l
c
c     Four correlated histogram slots, all filled at the same FO PS point
c     so the K_EW ratio benefits from MC-stat cancellation:
c       i=1  NLO    : full NLO total -- no filter, sum of all amp_split
c                    contributions. NUMERATOR of K_EW.
c       i=2  LO     : Born only (any tag); ibody=3.
c       i=3  NLOQCD : NLO contributions with squared alpha_EW = 2;
c                    ibody.ne.3 AND orders_tag_plot/100==2.
c       i=4  NLOEW  : NLO contributions with squared alpha_EW >= 4;
c                    ibody.ne.3 AND orders_tag_plot/100>=4.
c     The slot algebra is exhaustive and disjoint:
c          i=2 + i=3 + i=4 == i=1   (algebraically, modulo MC noise).
c     K_EW = i=1 / (i=2 + i=3).
c
c     PHYSICAL INTERPRETATION (no Born restriction in the generation):
c     The integrand contains all Born coupling combinations
c     (alpha_s^N alpha^1, alpha_s^(N-1) alpha^2, ..., alpha_s^0 alpha^(N+1))
c     and all NLO QCD + NLO EW corrections on each. Therefore:
c       - i=3 receives the QCD-Born plus its NLO QCD correction; this is
c         NOT equal to a separate `[QCD]` run because it omits NLO QCD
c         on the EW Born (which has qed_squared >= 4 already).
c       - i=4 mixes (a) NLO_EW on QCD-Born, (b) NLO_QCD on EW-Born
c         (degenerate with (a) at qed_squared=4), (c) NLO_EW on EW-Born
c         (qed_squared=6), and any higher-EW pieces. Cannot be cleanly
c         labelled as "NLO EW" alone -- the |T@NLOEW name is retained
c         for downstream-tool compatibility, not as a physics statement.
c     If you want clean "pure NLO QCD" vs "pure NLO EW" labels, restore
c     the `aS=N aEW=1` Born restriction in the generate command (and
c     re-tighten i=4 to qed_squared==4).
c
c     orders_tag_plot encoding (set in /corderstagplot/ by
c     get_orders_tag in splitorders_stuff.f):
c          tag = ord_QCD_squared + 100 * ord_QED_squared
c     so qed_squared = orders_tag_plot / 100 in integer division. The
c     squared QED power is always even (amplitudes have integer alpha_EW
c     vertex counts), so qed_squared = 2 and qed_squared >= 4 partition
c     the NLO phase space.
c
      character*13 cc(4)
      data cc/ ' |T@NLO      ',' |T@LO       ',' |T@NLOQCD   '
     $        ,' |T@NLOEW    '/
...
c     Defensive guard: this MG5 branch can emit EW-Sudakov contributions
c     with ibody = 10, 11, 12 (LSC/SSC/..., see fks_singular.f:188-199
c     and madfks_plot.f:260, ibody = 10 + itype - 100). With the bare
c     loop_qcd_qed_sm_Gmu model and a plain [QCD QED] run those code
c     paths do not link, so this guard is dead code in practice -- left
c     in as defense in depth in case the model is overridden via $MODEL.
      if (ibody.ge.10) return
...
c
     ...
     $        then
      ...
      do i=1,4
         l=(i-1)*59
c
c        Slot filtering -- see analysis_begin for the full doc.
c          i=1 (NLO)    : numerator -- no filter, every contribution.
c          i=2 (LO)     : Born only, ibody=3 (any qed_squared).
c          i=3 (NLOQCD) : qed_squared==2 AND ibody.ne.3.
c          i=4 (NLOEW)  : qed_squared>=4 AND ibody.ne.3.
c        Slots are disjoint and exhaustive: i=1 == i=2+i=3+i=4
c        algebraically (i=2 catches every ibody=3, i=3/i=4 partition
c        the ibody.ne.3 part by qed_squared). Including ibody.ne.3 in
c        the i=4 cut is essential -- without it, the EW Born at
c        qed_squared=4 (e.g. gamma q -> Z q) would double-count
c        between i=2 and i=4.
c
         if (i.eq.2 .and. ibody.ne.3) cycle
         if (i.eq.3 .and. orders_tag_plot/100.ne.2) cycle
         if (i.eq.3 .and. ibody.eq.3) cycle
         if (i.eq.4 .and. orders_tag_plot/100.lt.4) cycle
         if (i.eq.4 .and. ibody.eq.3) cycle
